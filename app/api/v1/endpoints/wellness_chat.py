...
async def wellness_chat(
    ...
    # If the user reports symptoms (fever, high temperature, feeling unwell etc.), ask 2-3 clarifying questions first
    def _detect_symptom_topic(q: str):
        ql = q.lower()
        if "fever" in ql or "temperature" in ql or "temp" in ql:
            return "fever"
        if any(w in ql for w in ["not feeling", "not feel", "not good", "sick", "unwell", "pain", "hurt", "headache", "dizzy"]):
            return "symptom"
        return None

    followup_questions = {
        "fever": [
            "I'm sorry you're unwell — can you tell me how high your temperature is (in °C or °F) and when it started?",
            "Do you have any other symptoms like chills, cough, shortness of breath, rash, vomiting, or severe headache?",
            "Do you have any chronic conditions, are you on medications, or have you taken any fever-reducing medication yet?"
        ],
        "symptom": [
            "Sorry to hear that. Can you tell me more about what you're feeling (e.g., specific symptoms, severity, when it started)?",
            "Have you noticed any other symptoms or changes (fever, breathing difficulty, lightheadedness, nausea)?",
            "Are you on medications or do you have any conditions I should be aware of?"
        ]
    }

    user_info = all_info.get(user_id, {})
    clarification = user_info.get("clarification")

    # If we're mid-clarification, treat this user message as an answer
    if clarification and clarification.get("active"):
        # Save the user's answer
        clarification.setdefault("collected", []).append(query)
        clarification["round"] = clarification.get("round", 0) + 1
        # Allow 1-2 follow-ups (finalize after 3 answers). This keeps the flow short and conversational.
        max_rounds = 3
        topic = clarification.get("topic")

        # If we should ask another follow-up question
        if clarification["round"] < max_rounds and clarification["round"] < len(followup_questions.get(topic, [])):
            next_q = followup_questions[topic][clarification["round"]]
            # persist clarification state
            user_info["clarification"] = clarification
            all_info[user_id] = user_info
            response = next_q
        else:
            # Gather everything and request final recommendation from the model
            q_lines = [f"Initial: {clarification.get('initial')}\n"]
            for i, ans in enumerate(clarification.get("collected", [])):
                q_text = followup_questions.get(topic, [])[i] if i < len(followup_questions.get(topic, [])) else f"Follow-up {i+1}"
                q_lines.append(f"Q{i+1}: {q_text}\nA{i+1}: {ans}\n")
            final_query = "\n".join(q_lines) + "\nPlease provide concise, actionable recommendations and clear escalation criteria for urgent care when needed."
            response = chat_with_memory(final_query, user_id)
        ...
        # Not in clarification mode — detect whether we should start it
        topic = _detect_symptom_topic(query)
        if topic:
            # Start clarification flow and ask first question
            first_q = followup_questions[topic][0]
            user_info = all_info.get(user_id, {})
            user_info["clarification"] = {
                "active": True,
                "topic": topic,
                "round": 0,
                "initial": query,
            ...
            # Default: get immediate AI response
